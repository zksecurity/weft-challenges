...
theorem testBit_toNatAux : ∀ (n : Nat) (w : Fin n → GF2) (i : Nat),
  ...
  | n + 1, w, i + 1 => by
    ...
    by_cases h : i < n
    · rw [dite_eq_left h, dite_eq_left (Nat.succ_lt_succ h)]
      rfl
    · rw [dite_eq_right h, dite_eq_right (fun h' => h (Nat.lt_of_succ_lt_succ h'))]
...
theorem testBit_toNat (w : Word32) (i : Nat) (hi : i < 32) :
  ...
  rw [toNat, testBit_toNatAux, dite_eq_left hi]

theorem testBit_toNat_of_ge (w : Word32) (i : Nat) (hi : 32 ≤ i) : (toNat w).testBit i = false := by
  rw [toNat, testBit_toNatAux, dite_eq_right (by omega)]
...
theorem toNat_ch (x y z : Word32) : toNat (ch x y z) = ((toNat x &&& toNat y) ^^^ ((toNat x ^^^ 0xffffffff) &&& toNat z)) := by
  apply Nat.eq_of_testBit_eq
  intro i
  simp only [Nat.testBit_xor, Nat.testBit_and, testBit_mask, ch,
    testBit_toNat']
  by_cases h : i < 32
...
theorem toNat_maj (x y z : Word32) : toNat (maj x y z) = ((toNat x &&& toNat y) ^^^ (toNat x &&& toNat z) ^^^ (toNat y &&& toNat z)) := by
  apply Nat.eq_of_testBit_eq
  intro i
  simp only [Nat.testBit_xor, Nat.testBit_and, maj, testBit_toNat']
  by_cases h : i < 32
